...
MemoryCard::MemoryCard(System* system, u32 slot, std::string path, std::vector<u8> data)
  : m_system(system), m_path(std::move(path)), m_slot(slot)
{
...
bool MemoryCard::SaveIfChanged(bool display_osd_message)
{
  ...
  if (!m_system->GetHostInterface()->SaveMemoryCard(m_path, m_slot, m_data.data(), DATA_SIZE))
  {
